...
void init_idt() {
  ...
  // 0x11 即0001 0001
  // 0		0		0		1		0		0		0
  // 1 A7    A6      A5      1 （恒为1）     LTIM    ADI     SNGL    IC4
  // A7-A5表示在MCS80/85中用于中断服务过程的页面起始地址，与ICW2中的A15-A8共同组成
  // LTIM  1-电平触发中断  当8259A检测到
  // IRi（i＝0～7）端有高电平时产生中断。在这种触发方式中，要求触发电平必须保持到中断响应信号INTA有效为止。在CPU响应中断后，应及时撤销该请求信号，以防止CPU再次响应，出现重复中断现象。
  // LTIM  0-边缘触发中断	当 LTIM=0
  // 时，为边沿触发方式。当8259A检测到IRi端有由低到高的跳变（上升沿）信号时产生中断。
  // SNGL  1-单片   0-多片
  // IC4  1-需要ICW4   0-不需要
  outb(0x20, 0x11);
  outb(0xA0, 0x11);
  ...
  //主片的ICW3的8位代表的是连接从片的情况，哪一位为1就代表哪一位的IRx连接了从片，这里是IR2连接从片所以值为0x04(0000
  //0100)
  outb(0x21, 0x04);
  ...
  //这里设置成0x01。表示 8259A
  //芯片被设置成普通全嵌套、非缓冲、非自动结束中断方式，并且用于 8086
  //及其兼容系统。 设置芯片以8086方式工作
  outb(0x21, 0x01);
  outb(0xA1, 0x01);
  ...
  // 0-32:  用于 CPU 的中断处理
  // 目标代码段为gdt表中下标为1的位置，所以是0x08,一个描述符8字节。
  //                                段存在标志p存在，
  //                                DPL特权级别为0,剩下五位01110表示中断门，01111表示陷阱门，00101表示任务门，这里是中断门。
...
void irq_handler(pt_regs *regs) {
  ...
  // D5：SL=0时候，L2-L0无效 SL=1时候，L2-L0有效
  // 中断结束命令位。EOI＝1使当前ISR寄存器的相应位清0。当ICW4中的AEOI为0时，ISR中的相应置位就要由该命令位来清除。
  // D4-D3：0 0   OCW2标志
